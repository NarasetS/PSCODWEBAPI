...
def obtaindata(startrowid, endrowid, storingpath):
    ...
    df.to_csv(storingpath+str("From_" + str(startrowid) + "_to_" + str(endrowid) + "_Row_ExistingPlant.csv"),encoding="utf-8-sig",index=False)

    return None

def combinerawdata(storingpath) :
    scrappingfilespath = storingpath
    scrappingdata = pd.DataFrame()

    for i in os.listdir(scrappingfilespath):
        scrappingfile = pd.read_csv(storingpath+i)
    ...
    print(scrappingdata)
    scrappingdata.to_csv(storingpath+"\\ExistingPlants.csv",encoding="utf-8-sig",index=False)
